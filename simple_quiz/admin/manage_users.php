...
// Automatically deactivate users who haven't logged in for 30 days
$pdo->query("UPDATE users SET status = 'inactive' WHERE role = 'user' AND status = 'active' AND (last_login IS NULL OR last_login < DATE_SUB(NOW(), INTERVAL 30 DAY))");

// Handle user actions (update status, delete)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action && $user_id) {
        switch ($action) {
            ...
            case 'delete':
                // Delete user's answers (by submission_id, since user_answers does NOT have user_id)
                $stmt = $pdo->prepare("DELETE FROM user_answers WHERE submission_id IN (SELECT id FROM user_submissions WHERE user_id = ?)");
                $stmt->execute([$user_id]);

                // Delete user's quiz submissions
                $stmt = $pdo->prepare("DELETE FROM user_submissions WHERE user_id = ?");
                $stmt->execute([$user_id]);
...
// First, create a temporary table with user scores
$pdo->query("
...
// Now fetch the results with rankings
$stmt = $pdo->query("
    SELECT 
        ur.*,
        (
            ...
            WHERE ur2.avg_score > ur.avg_score
        ) as rank
    FROM user_rankings ur
    ORDER BY avg_score DESC, total_quizzes_taken DESC
");
...
<head>
    ...
    <title>Manage Users - Admin - Quizify</title>
    ...
    <style>
        ...
        * {
        ...
        .users-table tr:hover .username:after {
            transform: scaleX(1);
        }
        ...
        @media (max-width: 768px) {
            ...
            .stats-badge, .status-badge, .rank-badge {
                padding: 0.5rem 0.75rem;
                font-size: 0.75rem;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const searchStats = document.getElementById('userCount');
            ...
            searchInput.addEventListener('input', function() {
                ...
                // Update stats
                if (searchTerm) {
                    searchStats.textContent = `${visibleCount} of ${totalUsers} users`;
                } else {
                    searchStats.textContent = `${totalUsers} users`;
                }
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="stats-badge quizzes">
                                        ...
                                        <?=htmlspecialchars($user['total_quizzes_taken'])?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span class="stats-badge date">
                                        ...
                                        <?=htmlspecialchars($user['last_login'] ? date('Y-m-d H:i', strtotime($user['last_login'])) : 'Never')?>
                                    </span>
                                </td>
                                <td>
